...
// Khởi tạo trạng thái Giỏ hàng & Yêu thích (Fix Bug ngầm)
let cart = JSON.parse(storage.get('sgu_cart')) || [];
let wishlist = JSON.parse(storage.get('sgu_offline_wishlist')) || [];
...
const CONFIG = {
  USE_MOCK_API: false, 
API_BASE_URL: 'https://a10.nhahodau.net/API'
};
...
const api = {
  async request(endpoint, method = 'GET', body = null) {
    ...
    try {
      ...
      catch (e) { data = { message: 'Lỗi phản hồi từ server (Server Error)' }; }
...
async function initAuthState() {
  ...
  if (isLoggedIn) {
    ...
    if(res.status === 'success' && res.data) wishlist = res.data;
...
document.getElementById('user-btn')?.addEventListener('click', (e) => { 
  e.stopPropagation();
  if (!isLoggedIn) openModal(document.getElementById('auth-modal')); 
  else if (window.innerWidth <= 768) document.getElementById('mobile-dropdown').classList.toggle('mobile-show'); 
});

document.addEventListener('click', (e) => {
  const userWrapper = document.getElementById('user-wrapper'); const mobileDropdown = document.getElementById('mobile-dropdown');
  if (userWrapper && mobileDropdown && !userWrapper.contains(e.target)) mobileDropdown.classList.remove('mobile-show');
});
...
document.getElementById('logout-action')?.addEventListener('click', () => { 
  if(confirm("Bạn có chắc chắn muốn đăng xuất?")) { 
    storage.remove('sgu_token'); storage.remove('sgu_username');
    isLoggedIn = false; currentUser = ""; mockApiAddress = ""; savedPhone = "";
    updateAuthUI(); cart = []; updateCartUI(); wishlist = []; updateWishlistUI();
    showCustomToast("<i class='ph-light ph-sign-out' style='margin-right:8px; font-size: 18px; vertical-align: middle;'></i> Đã đăng xuất!", "success");
  } 
});
...
document.getElementById('login-form')?.addEventListener('submit', async (e) => { 
  ...
  try {
    ...
    const username = document.getElementById('login-user').value.trim(); const pass = document.getElementById('login-pass').value;
    ...
    if (response.status === 'success') {
      ...
      e.target.reset(); showCustomToast("<i class='ph-light ph-check-circle' style='margin-right:8px; font-size: 18px; vertical-align: middle;'></i> Đăng nhập thành công!", "success");
    } else { showCustomToast("<i class='ph-light ph-warning-circle' style='margin-right:8px; font-size: 18px; vertical-align: middle;'></i> " + response.message, "error"); }
  } catch (err) { showCustomToast("Lỗi JS: " + (err.message || "Crash ẩn!"), "error"); } 
  finally { if (btn) { btn.innerHTML = originalText; btn.style.pointerEvents = "auto"; btn.style.opacity = "1"; } }
});
...
document.getElementById('register-form')?.addEventListener('submit', async (e) => { 
 ...
 // Lấy thêm Giới tính từ form Đăng ký (Tickbox)
    ...
    const userData = { 
      ...
      address: document.getElementById('reg-street').value.trim(), 
    ...
    if (response.status === 'success') { 
      showCustomToast("<i class='ph-light ph-check-circle'></i> Đăng ký thành công! Hãy đăng nhập.", "success"); 
      e.target.reset(); 
...
function processFilters() {
  ...
  // 1. LỌC THEO DANH MỤC & MỨC GIÁ
  if (currentCat !== 'all') {
      if (currentCat.startsWith('gia-')) {
          // Lọc bằng giá tiền thực tế (chấp cả DB Laravel)
          if (currentCat === 'gia-89k') {
              temp = temp.filter(p => p.price >= 89000 && p.price < 199000);
          } else if (currentCat === 'gia-199k') {
              temp = temp.filter(p => p.price >= 199000 && p.price < 500000);
          } else if (currentCat === 'gia-500k') {
              temp = temp.filter(p => p.price >= 500000);
          }
      } else {
          // Lọc theo thuộc tính (Hoa tươi, hoa sáp, dạng bó, dạng hộp...)
          temp = temp.filter(p => {
              // Phòng hờ nếu vẫn đang xài mảng categories của Mock API
              if (p.categories && p.categories.includes(currentCat)) return true;
              
              // Map chuẩn 100% với cấu trúc Database Laravel của bạn
              let cId = p.category_id || "";
              let cUnit = p.unit || "";
              
              if (currentCat === 'tuoi') return cId === 'tuoi';
              if (currentCat === 'sap') return cId === 'sap';
              if (currentCat === 'tuoi-bo') return cId === 'tuoi' && cUnit === 'bó';
              if (currentCat === 'tuoi-ke') return cId === 'tuoi' && cUnit === 'kệ';
              if (currentCat === 'sap-bo') return cId === 'sap' && cUnit === 'bó';
              if (currentCat === 'sap-hop') return cId === 'sap' && cUnit === 'hộp';
  ...
  // 2. LỌC THEO SEARCH BOX (Không được làm mất từ khóa hiện tại)
  ...
  // 3. Lọc theo thanh kéo thả khoảng giá (nếu có)
...
function renderProducts() {
  ...
  if (filteredData.length === 0) { grid.innerHTML = '<p style="color:#aaa; grid-column: 1/-1; text-align:center; font-size:18px;">Không tìm thấy sản phẩm nào phù hợp.</p>'; return; }
  ...
  pageItems.forEach(item => {
    ...
    const cardHTML = `
      <div class="card show dynamic-card" data-title="${item.name}" data-price="${formatVND(item.price)}" data-desc="${item.description || item.subtitle}" data-images="${imgUrl}">
...
function renderPagination() {
  ...
  pagination.insertAdjacentHTML('beforeend', `<li class="${currentPage === 1 ? 'disabled' : ''}" onclick="goToPage(${currentPage - 1})"><i class="ph-light ph-caret-left"></i></li>`);
  for (let i = 1; i <= totalPages; i++) { pagination.insertAdjacentHTML('beforeend', `<li class="${currentPage === i ? 'active' : ''}" onclick="goToPage(${i})">${i}</li>`); }
  pagination.insertAdjacentHTML('beforeend', `<li class="${currentPage === totalPages ? 'disabled' : ''}" onclick="goToPage(${currentPage + 1})"><i class="ph-light ph-caret-right"></i></li>`);
}
...
document.querySelectorAll('.cat-item').forEach(li => {
  li.addEventListener('click', function(e) {
    e.stopPropagation(); document.querySelectorAll('.cat-item').forEach(el => el.classList.remove('active')); this.classList.add('active'); 
    if(this.closest('.cat-dropdown')) this.closest('.has-dropdown').classList.add('active');
    currentCat = this.getAttribute('data-cat'); processFilters();
  });
});
...
if (searchInputEl) {
  searchInputEl.addEventListener('input', () => {
    ...
    searchTimeout = setTimeout(processFilters, 300);
  });
}
...
function handleCardClick(card) {
  ...
  if (document.getElementById('modal-subtitle') && card.querySelector('.card-subtitle')) document.getElementById('modal-subtitle').textContent = card.querySelector('.card-subtitle').textContent;
  if(document.getElementById('modal-price')) document.getElementById('modal-price').textContent = card.getAttribute('data-price'); 
  if(document.getElementById('modal-desc')) document.getElementById('modal-desc').textContent = card.getAttribute('data-desc');
...
function updateCartUI() {
  storage.set('sgu_cart', JSON.stringify(cart)); 
...
document.getElementById('cart-btn')?.addEventListener('click', (e) => { e.stopPropagation(); document.getElementById('wishlist-sidebar')?.classList.remove('open'); document.getElementById('cart-sidebar')?.classList.add('open'); });
document.querySelector('.close-cart')?.addEventListener('click', () => { document.getElementById('cart-sidebar')?.classList.remove('open'); });

document.getElementById('wishlist-menu-item')?.addEventListener('click', (e) => { e.stopPropagation(); renderWishlistUI(); document.getElementById('cart-sidebar')?.classList.remove('open'); document.getElementById('wishlist-sidebar')?.classList.add('open'); document.getElementById('mobile-dropdown')?.classList.remove('mobile-show'); });
document.querySelector('.close-wishlist')?.addEventListener('click', () => { document.getElementById('wishlist-sidebar')?.classList.remove('open'); });

document.addEventListener('click', (e) => {
  const cartSidebar = document.getElementById('cart-sidebar'); const wishlistSidebar = document.getElementById('wishlist-sidebar');
  if (cartSidebar && cartSidebar.classList.contains('open') && !cartSidebar.contains(e.target) && !e.target.closest('#cart-btn') && !e.target.closest('.add-cart-btn') && !e.target.closest('.add-to-cart')) { cartSidebar.classList.remove('open'); }
  if (wishlistSidebar && wishlistSidebar.classList.contains('open') && !wishlistSidebar.contains(e.target) && !e.target.closest('#wishlist-menu-item') && !e.target.closest('.fav-btn')) { wishlistSidebar.classList.remove('open'); }
});

/* =========================================
   10. TÍNH TOÁN GIÁ TIỀN & PHÍ SHIP (HỎA TỐC)
   ========================================= */
window.calculateOrderTotal = function() {
    let rawTotal = parseInt(document.getElementById('total-price')?.textContent.replace(/\D/g, '') || 0);
    let discount = 0;
    if(rawTotal >= 1000000) discount = 50000;
    ...
    let finalTotal = rawTotal - discount + shippingFee;
    ...
    if(shipEl) {
        shipEl.textContent = shippingFee > 0 ? formatVND(shippingFee) : 'Miễn phí';
        shipEl.style.color = shippingFee > 0 ? '#ffb6c1' : '#fff'; // Đổi màu cho nổi
    }
...
window.openCheckoutModal = async () => {
  ...
  if (!isLoggedIn) { 
    showCustomToast("<i class='ph-light ph-user' style='margin-right:8px; font-size: 18px; vertical-align: middle;'></i> Vui lòng đăng nhập để thanh toán!", "error"); 
  ...
  const originalText = checkoutBtn ? checkoutBtn.innerHTML : 'Tiến Hành Thanh Toán';
  if (checkoutBtn) {
    checkoutBtn.innerHTML = '<i class="ph-light ph-spinner fa-spin"></i> Đang tải dữ liệu...';
  ...
  try {
    ...
    let checkoutPhone = savedPhone || localStorage.getItem('userPhone') || "";
    let checkoutAddress = mockApiAddress || localStorage.getItem('userAddress') || "";

if (profileRes && profileRes.status === 'success') {
      let uData = profileRes.user || profileRes.data || {};
      checkoutName = uData.full_name || uData.name || localStorage.getItem('userFullname') || currentUser;
      checkoutPhone = uData.phone || checkoutPhone;
      ...
      checkoutAddress = adr.length > 0 ? adr.join(', ') : checkoutAddress;
      savedPhone = checkoutPhone; mockApiAddress = checkoutAddress;
    }
    calculateOrderTotal();

    if(checkoutAddress && checkoutPhone) { 
      if(document.getElementById('saved-address-info')) document.getElementById('saved-address-info').innerHTML = `<h4>Địa chỉ đã lưu</h4><p><strong id="saved-name-display">${checkoutName}</strong> | ${checkoutPhone}</p><p>${checkoutAddress}</p>`; 
      const rd = document.querySelector('input[name="address_type"][value="saved"]'); if(rd) rd.checked = true;
    } else { 
      if(document.getElementById('saved-address-info')) document.getElementById('saved-address-info').innerHTML = `<h4>Địa chỉ đã lưu</h4><p style="color: #ff6b81; font-size: 13px; font-style: italic; margin-top: 5px;">❌ Bạn chưa lưu địa chỉ/SĐT. Vui lòng chọn nhập địa chỉ mới bên dưới.</p>`; 
      const rd2 = document.querySelector('input[name="address_type"][value="new"]'); if(rd2) rd2.checked = true; 
    }
    ...
    showCustomToast("Lỗi tải thông tin, vui lòng thử lại!", "error");
  } finally {
    ...
    try {
        ...
        if (addressType === 'new') {
            ...
            if(!shippingName || !shippingPhone || !shippingCity || !shippingDistrict || !shippingWard || !shippingStreet) {
                ...
                else alert("Vui lòng điền đủ thông tin địa chỉ!");
        ...
        // NHÉT KÈM CHỮ HỎA TỐC VÀO TÊN ĐỂ QUA MẶT DATABASE
        const isExpress = (shippingMethodSelect === 'express');
        const finalName = shippingName + (isExpress ? ' [HỎA TỐC]' : '');
...
// TẠO MÃ ĐƠN HÀNG
        ...
        const orderData = {
            ...
            // SỬA DÒNG NÀY: Dùng trực tiếp currentUser vì trong DB nó chính là Email rồi
            customer_email: currentUser || 'khach@sguflower.com',
...
function renderFeaturedProducts() {
  ...
  const hotFlowers = ['Blue Dream', 'Autumn Whisper', 'Spring Awakening', 'Rose Passion'];
  const featuredItems = productsData.filter(p => hotFlowers.includes(p.name) || hotFlowers.includes(p.product_name)).slice(0, 4); 
  ...
  featuredItems.forEach(item => {
    ...
    // Mã hóa cực mạnh để chống vỡ cấu trúc thẻ HTML khi có ký tự lạ
    ...
    // Đã xóa onclick="handleCardClick(this)" để nhường việc cho bộ lắng nghe tổng của form
    html += `
      <div class="card show dynamic-card" data-title="${itemName}" data-price="${formatVND(itemPrice)}" data-desc="${itemDesc}" data-images="${imgUrl}">
        <div class="img-wrapper">
          ...
          <img class="card-img" src="${imgUrl}" loading="lazy" alt="${itemName}">
        </div>
        <div class="card-info">
          <h3>${itemName}</h3>
          <p class="card-subtitle">${rawDesc}</p>
          <div class="card-bottom">
...
// Chuyển hóa Data Laravel cho khớp 100% với giao diện cũ
          productsData = rawData.map(p => ({
              ...p,
              name: p.name || p.product_name, // Đảm bảo bắt đúng tên
              subtitle: p.description || p.subtitle || "Hoa SGU Flower",
              
              // SỬA DÒNG NÀY: Thêm p.image để bắt đúng tên cột từ Database
